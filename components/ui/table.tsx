...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-lg border border-border/50 bg-card/30"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "hover:bg-muted/30 data-[state=selected]:bg-primary/10 border-b border-border/30 transition-colors duration-150",
        className
      )}
      {...props}
    />
